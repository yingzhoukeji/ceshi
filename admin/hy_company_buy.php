...
@$uid = $_GET['uid'];
// 总记录数
$sql = "SELECT id  FROM  qb_buy_content where uid=$uid";
...
// 查询语句
$sql = "SELECT *  FROM  qb_buy_content where uid=$uid ORDER BY id DESC LIMIT " . $num . ",$fnum";
$result = doresult($sql);
...
// 翻页链接
for ($i = 0; $i < $pagenum; $i ++) {
    echo "<a href=hy_company_buy.php?uid=$uid&page=" . ($i + 1) . ">" . ($i + 1) . "</a>";
}
